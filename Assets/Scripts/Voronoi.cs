...
public class Voronoi
{
    ...
    public List<GraphEdge> generateVoronoi(double[] xValuesIn, double[] yValuesIn, double minX, double maxX, double minY, double maxY, out List<Site> siteVector)
    {
        ...
        siteVector = new List<Site>();
        for (int i = 0; i < sites.Length; i++)
            siteVector.Add(sites[i]);
        return allEdges;
    }
    ...
    private void sort(double[] xValuesIn, double[] yValuesIn, int count)
    {
        ...
        sqrt_nsites = (int)Mathf.Sqrt((float)sn);
    ...
    private void clip_line(Edge e)
    {
        ...
        if (Mathf.Sqrt((float)((x * x) + (y * y))) < minDistanceBetweenSites)
        {
            return;
        }
        pxmin = borderMinX;
