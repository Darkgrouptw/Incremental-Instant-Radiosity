...
public class LightSource_Debug : MonoBehaviour
{
    #region Camera 方向參數
    Vector3 right, up;
    #endregion
    ...
    void Start()
    {
        ...
        DrawVoronoiDiagram(allEdge);

        Texture2D tempTexture = new Texture2D(width, height);
        Utils.matToTexture2D(VoronoiDiagram, tempTexture);
        VD.texture = tempTexture;
    ...
    {
        for (int i = 0; i < VPLPoints.Count; i++)
            Debug.DrawLine(this.transform.position, VPLPoints[i]);
    }
    ...
    public void setDirection(Vector3 d)
    {
        ...
        this.gameObject.transform.rotation = Quaternion.LookRotation(d, right);
